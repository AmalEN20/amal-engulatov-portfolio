...
Cold load restores the approved preloader choreography from the earlier Amal portfolio while adapting its surface and message to the current direction. The sequence remains `holding → text-exiting → curtain-exiting → complete`: a fixed white surface, centered single-line black message (`Passion to build something new and interesting.`), the existing line entrance, text exit at approximately 1250ms, curtain departure beginning around 1780ms, and complete removal around 3020ms. Instrument Sans is used at a fluid size that preserves one line at 390px, 430px, and desktop widths; the direction and pacing remain Amal's own previous implementation.
...
Opening is finite and interaction-owned: backdrop approximately 680ms, panel approximately 1080ms on desktop and 1180ms on mobile, with a slow-start curtain curve so the surface does not appear almost complete in its first frames. The single clipping surface owns the entire entrance, so no information can lead or lag behind the card. It never depends on page scroll position and does not restart the site preloader. Closing is the exact reverse curtain: the same panel clip contracts from bottom to top over approximately 1080ms on desktop and 1180ms on mobile while every child retains full opacity and its final transform. The backdrop resolves separately without altering panel content. Native `dialog.close()` runs only after the panel clip animation completes (with a timeout used only as a fail-safe), preventing top-layer removal one frame before the curtain settles. Reopening a project intentionally replays only that dialog's surface entrance.
...
- Cold-load preloader state/timers and intro lock signal: one scoped client component; preloader surface/text and no-JS visibility fallback: CSS; independent watchdog: one root-layout script.
- Project-dialog semantics, focus containment, Escape behavior, and top-layer backdrop: native HTML `dialog`; its single finite curtain: CSS; its animation-completion listener, fail-safe timer, exact body-style restoration, trigger-focus restoration, and lock signal: one scoped client component. The existing `HeavyScroll` owner alone responds to intro/dialog lock signals by stopping, synchronizing, and restarting Lenis.
...
- Project dialogs: centered and comfortably inset on desktop; on mobile they retain a visible page margin, use the deliberately slower 1180ms curtain in both directions, and cap their internal height below the viewport rather than becoming full-screen.
...
- Cold load and refresh render the white preloader with one unwrapped black message, then preserve the approved hold, text exit, curtain exit, and final removal; intermediate frames never expose a stuck or clickable surface.
...
- Closing keeps all child text, dots, dividers, and actions fully opaque while the panel clip reverses; it completes the curtain before native top-layer removal, with no last-frame snap, flash, or stale Lenis coordinate.
