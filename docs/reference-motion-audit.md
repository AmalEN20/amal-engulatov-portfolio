...
- Repeated cold-load sampling on Cuberto home, Services, and About observed inner words beginning about 136px below a roughly 113px mask (approximately 120%), staying at opacity 1, and settling near zero over roughly 1.4–1.7 seconds with about 80–100ms neighboring overlap.
- Cuberto applies the same word-mask construction to supporting paragraphs: each word has its own clipped outer wrapper and padded inner transform layer. Supporting copy starts while the heading is still settling rather than after it fully finishes.
- On Cuberto home, below-fold H2 words remained at their 120% start transform after the cold-load hero had completed and began resolving only when their section approached the viewport. This confirms a section-entry behavior distinct from the requested Amal behavior.
...
- Cuberto's H1 keeps one accessible label while visual word fragments are `aria-hidden`. Every word has an outer `overflow: clip` wrapper, a small negative outer margin, and a padded inner inline block, protecting final descenders.
...
| Supporting-copy overlap | Cuberto home/services | Avoid waiting for spectacle | Per-word masks begin once heading is partly readable | DOM transforms, observed and confirmed | Low-medium | All portfolio supporting text uses the same per-word grammar with tighter stagger |
...
The new structure remains semantic DOM/CSS. One Lenis instance is justified only by Amal's explicit heavy-scroll direction; it owns desktop/fine-pointer wheel inertia and one RAF, while touch and reduced motion remain native. The two-column desktop relationship uses Amal's existing pixel portrait Canvas in place of the non-transferable illustration. The canvas is fixed to the viewport bottom, has no visible frame, and receives no scroll progress. The audited word-mask and supporting-copy mechanics are implemented with finite CSS animations. Cuberto's observed viewport-triggered below-fold behavior is deliberately not transferred because Amal explicitly requested a single complete page-load entrance: every below-fold group animates once on entry and is already finished when later scrolled into view. Lines use the same entry clock and scale from left to right. GSAP is not justified because this pass contains no scrubbed scene or scroll-owned choreography. WebGL, video, body locks, view timelines, and threshold observers remain excluded.
